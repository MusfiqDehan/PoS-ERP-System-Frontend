...
> Derived by reading the actual fixtures. Field names are **verbatim from the fixtures** (including quirks/typos like `creadedon`, `Valitidy`, `customer_image`) so the UI needs no column renames. Fix typos later via a serializer `source=` if desired, but ship matching first.
...
## 1. Auth & Tenancy  *(not fixture-backed — build fresh)*
...
| Low stock | `/products/low-stock/` *(computed)* | `img, warehouse, store, product, category, sku, qty, qtyalert` |
| Expired | `/products/expired/` *(computed)* | `img, product, sku, manufactureddate, expireddate` |
...
## 5. POS  *(transactional — builds on Products + Customers)*
...
## 11. Reports  *(read-only; mostly aggregations/filters of core models)*
...
## 15. Superadmin  *(POST-LAUNCH BACKLOG — manage via Django admin for v1)*
