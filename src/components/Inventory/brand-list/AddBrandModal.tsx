...
import type { CreateBrandPayload } from "@/lib/inventory";
...
type Props = {
  onAddBrand: (payload: CreateBrandPayload) => Promise<boolean>;
};

export default function AddBrandModal({ onAddBrand }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim()) return;
    setSubmitting(true);
    const success = await onAddBrand({ name: name.trim(), is_active: isActive });
    setSubmitting(false);
    if (success) {
      resetForm();
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content">
          <div className="flex items-center justify-between p-4 border-b border-[#f1f1f1]">
            <h4 className="m-0 text-[18px] font-bold text-[#212B36]">Add Brand</h4>
            <button
              type="button"
