...
export default function DeleteBrandModal({ brand, onDeleteBrand }: Props) {
  const [deleting, setDeleting] = useState(false);

  const handleDelete = async () => {
    if (!brand) return;
    setDeleting(true);
    const success = await onDeleteBrand(brand.id);
    setDeleting(false);
    if (success) {
      (window as any).bootstrap?.Modal?.getInstance?.(
        document.getElementById("delete-brand-modal"),
      )?.hide();
    }
  };

  return (
    <div className="modal fade" id="delete-brand-modal">
