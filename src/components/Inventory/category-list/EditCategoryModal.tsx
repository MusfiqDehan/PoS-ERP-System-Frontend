...
import type { UpdateCategoryPayload } from "@/lib/inventory";
...
type Props = {
  ...
  onEditCategory: (id: string, payload: UpdateCategoryPayload) => Promise<boolean>;
};

export default function EditCategoryModal({ category, onEditCategory }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!category || !name.trim() || !slug.trim()) return;
    setSubmitting(true);
    const success = await onEditCategory(category.id, {
      name: name.trim(),
      slug: slug.trim(),
      is_active: isActive,
    });
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content">
          <div className="flex items-center justify-between p-4 border-b border-[#f1f1f1]">
            <h4 className="m-0 text-[18px] font-bold text-[#212B36]">Edit Category</h4>
            <button
              type="button"
