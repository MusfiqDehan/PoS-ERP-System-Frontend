...
import type { Category, CreateCategoryPayload } from "@/lib/inventory";
...
type Props = {
  ...
  onAddSubCategory: (payload: CreateCategoryPayload) => Promise<boolean>;
};

export default function AddSubCategoryModal({ parentCategories, onAddSubCategory }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || !slug.trim() || !parentId) return;
    setSubmitting(true);
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content">
          <div className="flex items-center justify-between p-4 border-b border-[#f1f1f1]">
            <h4 className="m-0 text-[18px] font-bold text-[#212B36]">Add Sub Category</h4>
            <button
              type="button"
