...
import type { CreateUnitPayload } from "@/lib/inventory";
...
type Props = {
  onAddUnit: (payload: CreateUnitPayload) => Promise<boolean>;
};

export default function AddUnitModal({ onAddUnit }: Props) {
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content">
          <div className="flex items-center justify-between p-4 border-b border-[#f1f1f1]">
            <h4 className="m-0 text-[18px] font-bold text-[#212B36]">Add Unit</h4>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-end gap-2 p-4 border-t border-[#f1f1f1]">
              <button type="button" data-bs-dismiss="modal" className="px-4 py-2 rounded-[6px] border border-[#e7e7e7] text-[#646B72] text-[14px] font-medium hover:bg-[#f6f6f6] transition-colors" onClick={resetForm}>
                Cancel
              </button>
